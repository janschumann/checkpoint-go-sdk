...
var commonSecurity []map[string][]string

func init() {
	// init security property
	optionList := make([]string, 0)
	sec := make(map[string][]string)
	sec["apiKey"] = optionList
	commonSecurity = append(commonSecurity, sec)
}
...
func (c *converter) handleTags() {
	for _, tag := range c.tags.getTags() {
...
func (c *converter) createPathItem(command Command) spec.PathItem {
		...
		PathItemProps: spec.PathItemProps{
			Post: &spec.Operation{
				OperationProps: spec.OperationProps{
					...
					Tags:        []string{c.tags.getTagForCommand(command.Name)},
				},
			},
		},
	}
	...
	if command.Name != "login" {
		item.Post.Security = commonSecurity
	}
